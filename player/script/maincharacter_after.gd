...
# ถ้าระบบโจมตีของคุณมี Damage จริง
# ให้เพิ่มตรงนี้ด้วย
...
func _ready() -> void:
	...
	base_heal_rate = health_regen_per_second

	apply_shop_stats()

	if !PlayerStats.stats_updated.is_connected(apply_shop_stats):
		PlayerStats.stats_updated.connect(apply_shop_stats)
...
func start_cast_release(target: Node2D = null, damage_amount: int = 1) -> void:
	...
	pending_damage_amount = int(
		PlayerStats.get_final_damage(
			float(damage_amount)
		)
	)
...
func apply_shop_stats():

	speed = PlayerStats.get_final_speed(base_speed)

	max_hp = PlayerStats.get_final_max_hp(base_max_hp)

	max_stamina = PlayerStats.get_final_max_stamina(base_max_stamina)

	health_regen_per_second = PlayerStats.get_final_heal_rate(base_heal_rate)

	# ป้องกัน HP เกิน Max ใหม่
	hp = min(hp, max_hp)
	...
	if status_bars != null:
		...
		if status_bars.has_method("setup"):
