...
func _process(delta: float) -> void:
	...
	for child in get_children():
		if child is Parallax2D:
			...
			# Wrap X
			if layer.repeat_size.x > 0:
				while layer.scroll_offset.x <= -layer.repeat_size.x:
					layer.scroll_offset.x += layer.repeat_size.x

				while layer.scroll_offset.x >= layer.repeat_size.x:
					layer.scroll_offset.x -= layer.repeat_size.x

			# Wrap Y
			if layer.repeat_size.y > 0:
				while layer.scroll_offset.y <= -layer.repeat_size.y:
					layer.scroll_offset.y += layer.repeat_size.y
